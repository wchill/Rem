...
namespace Rem.Commands.MemeGen
{
    public class GruPlanMemeGen : ModuleBase
    {
        ...
        {
            _botState = state;
            _template = new TextMemeTemplate("GruPlan.png",
                new TextBoundingBox
                {
        ...
        public async Task GenerateGruPlanMeme(string step1, string step2, string step3, string step4)
        {
            using (Context.Channel.EnterTypingState())
            {
                try
                {
                    var filename = await _template.GenerateImage(Context, step1, step2, step3, step4);
