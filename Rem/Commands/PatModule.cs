...
namespace Rem.Commands
{
    public class PatModule : ModuleBase
    {
        ...
        private static readonly Dictionary<ImageType, Tuple<string, string>> TemplateStrings =
            new Dictionary<ImageType, Tuple<string, string>>
            {
        ...
        private async Task HandleImageCommand(ImageType type, IUser user)
        {
            ...
            {
                await ReplyAsync(
                    $"There are no {type.ToFriendlyString()} images in the database, add some with -addimage {type.ToFriendlyString()} <url>");
                return;
            }

            var imageUrl = await table.Where(image => image.Type == type).OrderBy(p => p.Id)
                .Skip(_random.Next(imageCount)).FirstAsync();

            var userInfo = user ?? Context.User;

            var builder = new EmbedBuilder();
            var templateStrings = TemplateStrings[type];
            var title =  userInfo == Context.User ?
                string.Format(templateStrings.Item1, userInfo.Username) :
                string.Format(templateStrings.Item2, userInfo.Username, Context.User.Username);
